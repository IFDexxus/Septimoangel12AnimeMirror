--スピード・ワールド 
function c120000053.initial_effect(c)
	...
	e2:SetCondition(c120000053.damcon)
	e2:SetOperation(c120000053.damop)
	c:RegisterEffect(e2)
	--add counter
	...
	e3:SetOperation(c120000053.ctop)
	c:RegisterEffect(e3)
	--activation
	...
	e4:SetValue(c120000053.aclimit)
	...
	e7:SetTarget(c120000053.aclimit2)
	c:RegisterEffect(e7)
	--lose counter
	local e8=Effect.CreateEffect(c)
	e8:SetDescription(aux.Stringid(120000053,0))
	...
	e8:SetOperation(c120000053.ctopx)
	c:RegisterEffect(e8)
	--
...
function c120000053.ctcon2(e,re)
	return re:GetOwner()~=e:GetOwner() and e:GetHandler():GetFlagEffect(110000000)==0
end
function c120000053.op(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	local tc2=Duel.GetFieldCard(1-tp,LOCATION_SZONE,5)	
	if tc==nil then
		Duel.MoveToField(e:GetHandler(),tp,tp,LOCATION_SZONE,POS_FACEUP,true)
		if tc2==nil then
			local token=Duel.CreateToken(tp,120000053,nil,nil,nil,nil,nil,nil)		
...
function c120000053.damcon(e,c)
	if c==nil then return true end
	return Duel.GetTurnCount()~=1
end
function c120000053.damop(e,tp,eg,ep,ev,re,r,rp)
