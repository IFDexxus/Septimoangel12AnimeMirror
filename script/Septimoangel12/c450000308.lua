--グランエルＡ
function c450000308.initial_effect(c)
	--selfdes
	...
	e1:SetCondition(c450000308.sdcon)
	c:RegisterEffect(e1)
	--equip
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c450000308.eqcon)
	e2:SetOperation(c450000308.eqop)
	c:RegisterEffect(e2)
	--Attack Synchro
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(450000308,0))
	e3:SetType(EFFECT_TYPE_QUICK_O)
	e3:SetCategory(CATEGORY_POSITION)
	e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e3:SetRange(LOCATION_ONFIELD)
	e3:SetCode(EVENT_FREE_CHAIN)
	e3:SetCondition(c450000308.atkcon)
	e3:SetTarget(c450000308.atktg)
	e3:SetOperation(c450000308.atkop)
	c:RegisterEffect(e3)
end
function c450000308.cfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsFaceup() and c:IsSetCard(0x3013)
end
function c450000308.sdcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsType(TYPE_MONSTER) and not Duel.IsExistingMatchingCard(c450000308.cfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c) and not c:IsCode(63468625)
end
function c450000308.eqcon(e,tp,eg,ep,ev,re,r,rp)
...
function c450000308.eqop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local sg=Duel.SelectTarget(tp,c450000308.cfilter,tp,LOCATION_MZONE,0,1,1,nil)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		if not Duel.Equip(tp,eq,tc,false) then return end
			...
			e1:SetValue(c450000308.eqlimit)
...
function c450000308.eqlimit(e,c)
	return not c:IsDisabled()
end
function c450000308.tgfilter(c)
	local ec=c:GetEquipTarget()
	return c:IsType(TYPE_SYNCHRO) and (ec:IsSetCard(0x3013) or ec:IsCode(63468625)) and c:GetFlagEffect(450000308)==0
end
function c450000308.atfilter(c)
	return c:IsType(TYPE_MONSTER)
end
function c450000308.atkcon(e,tp,eg,ep,ev,re,r,rp)
	local ph=Duel.GetCurrentPhase()
	return e:GetHandler():IsType(TYPE_MONSTER) and Duel.GetTurnPlayer()==tp and ph>=0x08 and ph<=0x20
	and (Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamageCalculated()) and Duel.IsExistingTarget(c450000308.tgfilter,tp,LOCATION_SZONE,0,1,nil,e,tp)
	and Duel.IsExistingTarget(c450000308.atfilter,tp,0,LOCATION_MZONE,1,nil,e,tp)
end
function c450000308.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_SZONE) end
	if chk==0 then return not e:GetHandler():IsStatus(STATUS_CHAINING) and Duel.IsExistingTarget(c450000308.tgfilter,tp,LOCATION_SZONE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c450000308.tgfilter,tp,LOCATION_SZONE,0,1,1,nil)
end
function c450000308.atkop(e,tp,eg,ep,ev,re,r,rp)
	local ts=Duel.GetFirstTarget()
	local atks=ts:GetTextAttack()
	local sg=Duel.SelectTarget(tp,c450000308.atfilter,tp,0,LOCATION_MZONE,1,1,nil)
	local tc=sg:GetFirst()
	...
	and not (tc:IsHasEffect(EFFECT_INDESTRUCTABLE_BATTLE) or tc:IsHasEffect(EFFECT_INDESTRUCTABLE_COUNT)) then
		...
		ts:RegisterFlagEffect(450000308,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
