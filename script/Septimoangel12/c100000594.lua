...
function c100000594.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(100000594,0))
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCost(c100000594.cost)
	e1:SetTarget(c100000594.target1)
	e1:SetOperation(c100000594.operation1)
	c:RegisterEffect(e1)
	--Activate2
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100000594,1))
	e2:SetType(EFFECT_TYPE_ACTIVATE)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetCost(c100000594.cost)
	e2:SetTarget(c100000594.target2)
	e2:SetOperation(c100000594.operation2)
	c:RegisterEffect(e2)
...
function c100000594.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.RegisterFlagEffect(tp,100000594,RESET_PHASE+PHASE_END+RESET_EVENT+0x1fe0000,0,1)
end
function c100000594.allfilter(c)
	return c:IsFaceup() and c:IsCode(100000595)
end
function c100000594.dfilter(c)
	return c:IsFaceup() and c:IsCode(100000590)
end
function c100000594.target1(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c100000594.allfilter,tp,LOCATION_SZONE,0,nil)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsFacedown,tp,LOCATION_SZONE,0,1,e:GetHandler()) 
	 and Duel.IsExistingMatchingCard(c100000594.dfilter,tp,LOCATION_SZONE,0,1,nil) 
	 and dg:GetCount()==0 and e:GetHandler():GetFlagEffect(1000005942)==0 end
	 e:GetHandler():RegisterFlagEffect(1000005941,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
end
function c100000594.operation1(e,tp,eg,ep,ev,re,r,rp)
...
function c100000594.filter(c,minseq,maxseq)
...
function c100000594.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	local dg=Duel.GetMatchingGroup(c100000594.allfilter,tp,LOCATION_SZONE,0,nil)
	if chk==0 then return Duel.IsExistingMatchingCard(c100000594.cfilter,tp,LOCATION_SZONE,0,1,e:GetHandler()) 
	and Duel.IsExistingMatchingCard(c100000594.dfilter,tp,LOCATION_SZONE,0,1,nil) and dg:GetCount()>0 end
	e:GetHandler():RegisterFlagEffect(1000005942,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
end
function c100000594.operation2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	Duel.RegisterFlagEffect(tp,100000590,RESET_CHAIN,0,1)
	if not Duel.IsExistingMatchingCard(c100000594.cfilter,tp,LOCATION_SZONE,0,1,c) then return end
	local dg=Duel.GetMatchingGroup(c100000594.allfilter,tp,LOCATION_SZONE,0,nil)
	if dg:GetCount()>0 then
		...
		sg=Duel.GetMatchingGroup(c100000594.filter,tp,LOCATION_SZONE,0,c,minseq,maxseq)
	end	
