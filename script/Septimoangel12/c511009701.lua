...
function c511009701.initial_effect(c)
	...
	e2:SetCost(c511009701.cost2)
	e2:SetTarget(c511009701.target2)
	e2:SetOperation(c511009701.operation)
	c:RegisterEffect(e2)
end
...
function c511009701.cost2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost()
		and Duel.IsExistingMatchingCard(c511009701.filter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Remove(e:GetHandler(),POS_FACEUP,REASON_COST)
	Duel.PayLPCost(tp,math.floor(Duel.GetLP(tp)/2))
end
function c511009701.target2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		local te=e:GetLabelObject()
		local tg=te:GetTarget()
		return tg and tg(e,tp,eg,ep,ev,re,r,rp,0,chkc)
